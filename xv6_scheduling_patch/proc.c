...
int max_tick_of_q[5] = {1, 2, 4, 8, 16}; //! allowed run_time in queue
// int q_max_time[5] = {1, 2, 3, 4, 5};     //! allowed wait_time in queue
    ...
    for (int i = 0; i < 5; i++)
    {
      if (queues[i] != 0)
      {
        ...
        if (queues[i]->curr_q_ticks >= max_tick_of_q[i] && i != 4) //it used up its time slice.. move to lower priority queue
        {
          ...
          if (!PLOT)
          {
            // cprintf("%d used up its time slice and demoted from queue %d to queue %d\n", queues[i]->pid, old_queue, queues[i]->queue);
              // cprintf("%d,%d,%d\n", ticks, queues[i]->pid, queues[i]->queue);
          }
        }
